...
from pytuga.repl.completer import PythonCompleter
from pytuga.repl.key_bindings import load_python_bindings, load_sidebar_bindings, load_confirm_exit_bindings
from pytuga.repl.layout import PythonPrompt, create_layout
from pytuga.repl.style import get_all_code_styles, get_all_ui_styles, generate_style
from pytuga.repl.utils import get_jedi_script_from_document, document_is_multiline_python
from pytuga.repl.validator import PythonValidator
...
class PythonInput(object):
    ...
    def __init__(self,
        ...
        self._python_prompt_control = _python_prompt_control or PythonPrompt(
            self)
        ...
        # When True, don't insert whitespace after newline.
        self.paste_mode = False
        # Ask for confirmation when Control-D is pressed.
    ...
    def add_key_binding(self):
        ...
        (Mostly useful for a .pytuga/config.py file, that receives
    ...
    def _create_buffer(self):
        ...
        python_buffer = Buffer(
            ...
            validator=SwitchableValidator(
                self._validator,
                Condition(
                    lambda: self.enable_input_validation)),
