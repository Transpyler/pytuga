#!/usr/bin/env python
"""
ptpython: Interactive Python tugashell.
Usage:
    ptpython [ --vi ]
...
Options:
    ...
    -i, --interactive=<filename> : Start interactive tugashell after executing this file.
...
from pytuga.repl.repl import embed, enable_deprecation_warnings, run_config
...
def run():
    ...
    # When a file has been given, run that, otherwise start the tugashell.
    if a['<file>']:
        sys.argv = [a['<file>']] + a['<arg>']
        six.exec_(compile(open(a['<file>'], "rb").read(), a['<file>'], 'exec'))

    # Run interactive tugashell.
    else:
