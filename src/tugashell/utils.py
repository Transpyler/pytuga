...
def has_unclosed_brackets(text):
    ...
    text = re.sub(
        r'''('[^']*'|"[^"]*")''',
        '',
        text)  # XXX: handle escaped quotes.!
...
def get_jedi_script_from_document(document, locals, globals):
    try:
        ...
        # Workaround Jedi issue #514: for
        # https://github.com/davidhalter/jedi/issues/514
        return None
    except KeyError:
        # Workaroud for a crash when the input is "u'", the start of a unicode
        # string.
        return None
...
def document_is_multiline_python(document):
    ...
    # If we just typed a colon, or still have open brackets, always insert a
    # real newline.
