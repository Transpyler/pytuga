...
from pytuga.repl.filters import HasSignature, ShowCompletionsMenu, ShowCompletionsToolbar, ShowSidebar, ShowLineNumbersFilter, ShowSignature, ShowDocstring
...
class SignatureToolbar(Window):
    ...
    def __init__(self, python_input):
        ...
        super(SignatureToolbar, self).__init__(
            ...
            filter=# Show only when there is a signature
    ...
    def __init__(
        ...
        def get_tokens(cli):
            ...
            if not python_input.vi_mode and cli.focus_stack.current == 'search':
                append(
                    (TB, '[Ctrl-G] Cancel search [Enter] Go to this position.'))
    ...
    return HSplit([
        VSplit([
            HSplit([
                FloatContainer(
                    ...
                    floats=[
                        ...
                        Float(
                            ...
                            left=1,
                            right=0,
                            content=PythonSidebarHelp(python_input)),
                    ]),
                ArgToolbar(),
                SearchToolbar(),
                SystemToolbar(),
                ValidationToolbar(),
                CompletionsToolbar(
                    extra_filter=ShowCompletionsToolbar(python_input)),
                ...
                Window(
                    ...
                    filter=HasSignature(python_input) & ShowDocstring(
                        python_input) & ~IsDone(),
                    height=D(max=12),
                ),
            ]),
